      ...
      (function(jStat3, Math2) {
        ...
        jStat3.extend(jStat3.cauchy, {
          pdf: function pdf(x, local, scale2) {
            ...
            return scale2 / (Math2.pow(x - local, 2) + Math2.pow(scale2, 2)) / Math2.PI;
          },
          cdf: function cdf(x, local, scale2) {
            return Math2.atan((x - local) / scale2) / Math2.PI + 0.5;
          },
          inv: function(p, local, scale2) {
            return local + scale2 * Math2.tan(Math2.PI * (p - 0.5));
          },
        ...
        jStat3.extend(jStat3.gamma, {
          ...
          mode: function mode(shape, scale2) {
            if (shape > 1) return (shape - 1) * scale2;
            return void 0;
          },
        ...
        jStat3.extend(jStat3.pareto, {
          pdf: function pdf(x, scale2, shape) {
            if (x < scale2)
              return 0;
            return shape * Math2.pow(scale2, shape) / Math2.pow(x, shape + 1);
          },
          cdf: function cdf(x, scale2, shape) {
            if (x < scale2)
              return 0;
            return 1 - Math2.pow(scale2 / x, shape);
          },
          ...
          median: function median(scale2, shape) {
            return scale2 * (shape * Math2.SQRT2);
          },
          mode: function mode(scale2) {
            return scale2;
          },
          variance: function(scale2, shape) {
            if (shape <= 2)
              return void 0;
        ...
        jStat3.extend(jStat3.weibull, {
          pdf: function pdf(x, scale2, shape) {
            if (x < 0 || scale2 < 0 || shape < 0)
              return 0;
            return shape / scale2 * Math2.pow(x / scale2, shape - 1) * Math2.exp(-Math2.pow(x / scale2, shape));
          },
          cdf: function cdf(x, scale2, shape) {
            return x < 0 ? 0 : 1 - Math2.exp(-Math2.pow(x / scale2, shape));
          },
          inv: function(p, scale2, shape) {
          ...
          mode: function mode(scale2, shape) {
            if (shape <= 1)
              return 0;
...
var A_MAX = 400;
var A_RES = 4.94;
var Q75_PEAK = import_jstat.default.invgamma.inv(0.75, A_RES, A_RES - 1);
var Q75_FLOOR = import_jstat.default.invgamma.inv(0.75, A_MAX, A_MAX - 1);
var mean = (p) => p.beta / (p.alpha - 1);
var scale = (p) => p.beta;
function solveQ25(q, target, m) {
  const f = (a) => import_jstat.default.invgamma.inv(q, a, (a - 1) * m) - target;
  const flow = f(A_MIN);
  const fhigh = f(A_MAX);
...
var F = {
  ...
  bounds(p) {
    return [0, 1.02 * import_jstat.default.invgamma.inv(BOUNDS_Q, p.alpha, p.beta)];
  },
  integ(p) {
    return [0, 1.05 * import_jstat.default.invgamma.inv(INTEG_Q, p.alpha, p.beta)];
  },
  pdf(p, x) {
    if (x <= 0) return 0;
    return import_jstat.default.invgamma.pdf(x, p.alpha, scale(p));
  },
  handles: [
    {
      kind: "center",
    ...
    },
    {
      ...
      color: "#8b5cf6",
      lineCls: "miqr",
      at: (p) => import_jstat.default.invgamma.inv(0.25, p.alpha, scale(p)),
      chip: () => "q25",
    ...
    },
    {
      ...
      color: "#8b5cf6",
      lineCls: "miqr",
      at: (p) => import_jstat.default.invgamma.inv(0.75, p.alpha, scale(p)),
      chip: () => "q75",
...
var inversegamma_default = createWidget(F, { pins: "left" });
export {
  inversegamma_default as default
};

...
var SIG_MAX = 1e6;
function t75(nu) {
  return import_jstat.default.studentt.inv(0.75, nu);
}
var TAIL_FACTOR = 5.2 / 1.96;
function tBounds(p) {
  return TAIL_FACTOR * p.sigma * import_jstat.default.studentt.inv(0.975, p.nu);
}
var F = {
  name: "halfstudentt",
  label: "HalfStudentT",
  ...
  pdf(p, x) {
    if (x <= 0) return 0;
    return 2 * import_jstat.default.studentt.pdf(x / p.sigma, p.nu) / p.sigma;
  },
  handles: [
    {
      kind: "center",
...
var halfstudentt_default = createWidget(F, { pins: "left" });
export {
  halfstudentt_default as default
};
